            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @if (auth()->user()->role === 'coach' || auth()->user()->role === 'admin')
                        <a href="{{ route('exercises.create') }}" class="text-blue-500 underline">Subir tutorial</a>
                    @endif
                    ...
                    <div class="flex flex-col gap-6 mt-4">
                        @foreach ($exercises as $exercise)
                            <div class="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow p-4 max-w-full">
                                ...
                                <p class="descripcion mb-4">{{ $exercise->description }}</p>
                                <a href="{{ route('exercises.show', $exercise->id) }}" class="text-green-500 hover:text-green-700 hover:underline">
                                    Ver Tutorial
                                </a>

                                @if (auth()->user()->role === 'coach' || auth()->user()->role === 'admin')
                                    <div class="mt-4 flex justify-between">
                                        <a href="{{ route('exercises.edit', $exercise->id) }}" class="text-blue-500 underline">Editar</a>
